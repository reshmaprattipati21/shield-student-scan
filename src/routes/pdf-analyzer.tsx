...
export const Route = createFileRoute("/pdf-analyzer")({
  ...
  head: () => ({
    meta: [
      { title: "AI PDF Offer Letter Analyzer — ScamShield" },
      {
        name: "description",
        content:
          "Upload appointment letters, internship descriptions, or training agreements to scan for hidden financial traps and fraudulent compliance clauses.",
      },
    ],
  }),
});
      ...
      <main className="mx-auto max-w-5xl px-6 py-12">
        ...
        <div className="mb-8">
          ...
          <p className="text-muted-foreground max-w-3xl">
            Upload appointment letters, internship descriptions, or training agreements to scan for
            hidden financial traps and fraudulent compliance clauses.
          </p>
        </div>
          ...
          {scanning ? (
            <div className="flex flex-col items-center gap-4">
              <div className="relative h-24 w-24">
                ...
                <div className="absolute inset-0 flex items-center justify-center text-cyan-300 font-semibold tabular-nums">
                  {progress}%
                </div>
              </div>
              <div className="text-sm text-cyan-200/80 tracking-wide max-w-md">
                Parsing document metadata and running semantic analysis on employment terms…
              </div>
            </div>
          ) : (
        ...
        {result && (
          <div className="mt-6 grid gap-6 md:grid-cols-[300px_1fr]">
            <div className="glass rounded-2xl p-6 flex flex-col items-center justify-center">
              ...
              <div className="mt-3 text-xs text-muted-foreground truncate max-w-[240px]">
                {result.file.name}
              </div>
            </div>
            <div className="glass rounded-2xl p-6">
              ...
              {result.hits.length === 0 ? (
                ...
                <div className="flex items-start gap-2 text-sm text-destructive mb-4">
                  ...
                  <span>
                    <span className="font-semibold">Critical Risk Flagged.</span> The uploaded text
              ...
              {result.hits.length > 0 && (
                <>
                  ...
                  <ul className="space-y-2">
                    {Array.from(new Map(result.hits.map((h) => [h.reason, h])).values()).map(
                      (h, i) => (
