...
export function RiskGauge({ score, risk }: { score: number; risk: Risk }) {
  ...
  return (
    <div className="flex flex-col items-center justify-center gap-y-3 w-full">
      ...
      <div className={`text-4xl font-bold tabular-nums leading-none ${c.glowClass}`}>
        {Math.round(animated)}
      </div>
      <div
        className={`text-xs font-semibold tracking-[0.28em] ${c.glowClass} ${risk === "High" ? "animate-pulse" : ""}`}
      >
        {c.label}
      </div>
    </div>
